...
public class APCharacterControllerEditor : Editor
{
	...
	public void OnSceneGUI () 
	{
		...
		if (oController.m_rangedAttacks.m_enabled && m_bShow)
		{
			for(int i = 0; i < oController.m_rangedAttacks.m_attacks.Length; i++)
			{
				APRangedAttack curAttack = oController.m_rangedAttacks.m_attacks[i];
	...
	public void DrawStartPos(APRangedAttack.AttackContext context, bool bDraw)
	{
		if(bDraw && !string.IsNullOrEmpty(context.m_anim))
		{
			...
			Vector3 dir = new Vector3(context.m_bulletDirection, 90f, 0f);
			Quaternion rot = Quaternion.Euler(oController.transform.TransformDirection(dir));
